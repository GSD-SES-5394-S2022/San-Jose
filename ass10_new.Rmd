...
```{r}
# Stacked (by purpose) barplot for transit lines, force ~N? Actually, histogram with brackets for ridership, and having the count of lines within each bracket as the frequency

# pivot wider or pivot longer

# ggplot(data=route_trip_by_purpose, aes(x=Sepal.Width,fill = Species)) + geom_histogram()
...
VMT <- PMT_car$total_SOV_miles[1] + 
       PMT_car$total_HOV_miles[1] / 2.72
...
#join all table and sum 
route_trip_by_purporse_alt<-left_join(route_trips_HBO_alt,route_trips_HBW_alt,by="route") %>%
  left_join(route_trips_NHB_alt,by="route")%>%
                        select(route,ridership_HBO,ridership_HBW,ridership_NHB)%>%
                        mutate(sum_ridership=ridership_HBO+ridership_HBW+ridership_NHB)
                        

write_csv(route_trip_by_purporse_alt, here("alternative",
              "data",
              "transit_route_trip_by_purporse_alt.csv"))
...
top_ridership_line <- data.frame(
  mode = c('HBO', 'HBW', 'NHB'),
  existing = c(
    route_trip_by_purpose[route_trip_by_purpose[which.max(route_trip_by_purpose$HBO),]],
    route_trip_by_purpose[route_trip_by_purpose[which.max(route_trip_by_purpose$HBW),]],
    route_trip_by_purpose[route_trip_by_purpose[which.max(route_trip_by_purpose$NHB),]]
              ),
  alternative = c(
    route_trip_by_purpose_alt[route_trip_by_purpose_alt[which.max(route_trip_by_purpose$HBO),]],
    route_trip_by_purpose_alt[route_trip_by_purpose_alt[which.max(route_trip_by_purpose_alt$HBW),]],
    route_trip_by_purpose_alt[route_trip_by_purpose_alt[which.max(route_trip_by_purpose_alt$NHB),]]
  ))
...
PMT_car_alt <- rbind(PMT_car_HBO_alt,PMT_car_HBW_alt,PMT_car_NHB_alt) %>%
  #summarize by sov hov
...
#Calculate VMT
```{r}
VMT_alt <- PMT_car_alt$total_SOV_miles[1] + 
       PMT_car_alt$total_HOV_miles[1] / 2.72
...
cond_diff <- data.frame(
  condition = c('Existing', 'Alternative'),
  VMT = c(VMT, VMT_alt),
  PMT_walk = c(PMT_walk, PMT_walk_alt),
  PMT_bike = c(PMT_bike, PMT_bike_alt)) %>% 
  ggplot(aes(x = condition, y = VMT, fill = condition)) +

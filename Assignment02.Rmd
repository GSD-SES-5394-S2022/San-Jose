...
veh <- data.frame(veh_own = c("0","1","2","3","4+"), count = c(sum(census$no_veh_hhE), sum(census$one_veh_hhE), sum(census$two_veh_hhE), sum(census$three_veh_hhE), sum(census$fourmore_veh_hhE)))
...
overall <- ggplot(census_alt, 
       aes(x = alt_wfh_emp/alt_total_emp,
           y = med_incomeE,
           size = no_veh_hhE/total_hhsE,
           alpha=0.5
           )) +
  geom_point() +
  scale_size_continuous(name = "percentage of wfh_change by median income") +
  scale_x_continuous(name = "percentage work-from-home employment",

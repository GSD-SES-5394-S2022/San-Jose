...
# HBO
SOV_const_HBO <- -0.9235  #-0.92343
HOV_const_HBO <- -0.35 #0.02259229
transit_const_HBO <- -1.9 #-4.067441
walk_const_HBO <- 1.6 #-1.606334
bike_const_HBO<- -1.46#-3.616955
 
# HBW
SOV_const_HBW <- 0.89 #1.132976
HOV_const_HBW <--0.83 #-2.108496
transit_const_HBW <--0.045 #-3.033151
walk_const_HBW <- 0.48 #-3.572114
bike_const_HBW<--0.005  #-2.715594

# NHB
SOV_const_NHB <- -0.6 # -0.5024781
HOV_const_NHB <--0.54 #-0.3174682
transit_const_NHB <- -1.37 #-3.409766
walk_const_NHB <-1.3 #-1.711975
bike_const_NHB<- -1.8 #-4.066457
...
existing_model <- model_compare2 %>%
  filter(purpose %in% c("HBO_model 2", "HBW_model 2", "NHB_model 2"))
...
#HBO
HBO_modeled_mode_by_purpose_1_alt <- tibble(
  purpose = "HBO_model 1 alt", 
  pct_bike = sum(skims_alt$n_bike_HBO) / 
                 sum(skims_alt$HBO_flow),
  pct_SOV = sum(skims_alt$n_SOV_HBO) / 
                sum(skims_alt$HBO_flow),
  pct_HOV = sum(skims_alt$n_HOV_HBO) / 
                sum(skims_alt$HBO_flow),
  pct_transit = sum(skims_alt$n_transit_HBO) / 
                sum(skims_alt$HBO_flow),
  pct_walk = sum(skims_alt$n_walk_HBO) / 
                sum(skims_alt$HBO_flow))

HBO_model_compare_alt <- rbind(mode_by_purpose, HBO_modeled_mode_by_purpose_1_alt) 

HBO_model_compare_alt

#HBW
HBW_modeled_mode_by_purpose_1_alt <- tibble(
  purpose = "HBW_model 1 alt", 
  pct_bike = sum(skims_alt$n_bike_HBW) / 
                 sum(skims_alt$HBW_flow),
  pct_SOV = sum(skims_alt$n_SOV_HBW) / 
                sum(skims_alt$HBW_flow),
  pct_HOV = sum(skims_alt$n_HOV_HBW) / 
                sum(skims_alt$HBW_flow),
  pct_transit = sum(skims_alt$n_transit_HBW) / 
                sum(skims_alt$HBW_flow),
  pct_walk = sum(skims_alt$n_walk_HBW) / 
                sum(skims_alt$HBW_flow))

HBW_model_compare_alt <- rbind(mode_by_purpose, HBW_modeled_mode_by_purpose_1) 

HBW_model_compare_alt

#NHB
NHB_modeled_mode_by_purpose_1_alt <- tibble(
  purpose = "NHB_model 1 alt", 
  pct_bike = sum(skims_alt$n_bike_NHB) / 
                 sum(skims_alt$NHB_flow),
  pct_SOV = sum(skims_alt$n_SOV_NHB) / 
                sum(skims_alt$NHB_flow),
  pct_HOV = sum(skims_alt$n_HOV_NHB) / 
                sum(skims_alt$NHB_flow),
  pct_transit = sum(skims_alt$n_transit_NHB) / 
                sum(skims_alt$NHB_flow),
  pct_walk = sum(skims_alt$n_walk_NHB) / 
                sum(skims_alt$NHB_flow))

model_compare_alt <- rbind(mode_by_purpose, 
                           HBO_modeled_mode_by_purpose_1_alt,
                           HBW_modeled_mode_by_purpose_1_alt,
                           NHB_modeled_mode_by_purpose_1_alt
                           ) 
...
# HBO
HBO_modeled_mode_by_purpose_alt2<- tibble(
  purpose = "HBO_model 2 alt", 
  pct_bike = sum(skims_alt$n_bike_HBO) / 
                 sum(skims_alt$HBO_flow),
  pct_SOV = sum(skims_alt$n_SOV_HBO) / 
                sum(skims_alt$HBO_flow),
...
model_compare_alt <- rbind(model_compare_alt, HBO_modeled_mode_by_purpose_alt2) 

model_compare_alt 

# HBW
HBW_modeled_mode_by_purpose_alt2 <- tibble(
  purpose = "HBW_model 2 alt", 
  pct_bike = sum(skims_alt$n_bike_HBW) / 
                 sum(skims_alt$HBW_flow),
  pct_SOV = sum(skims_alt$n_SOV_HBW) / 
                sum(skims_alt$HBW_flow),
...
model_compare_alt <- rbind(model_compare_alt, HBW_modeled_mode_by_purpose_alt2) 

model_compare_alt

# NHB
NHB_modeled_mode_by_purpose_alt2 <- tibble(
  purpose = "NHB_model 2 alt", 
  pct_bike = sum(skims_alt$n_bike_NHB) / 
                 sum(skims_alt$NHB_flow),
  pct_SOV = sum(skims_alt$n_SOV_NHB) / 
                sum(skims_alt$NHB_flow),
...
model_compare_alt <- rbind(model_compare_alt, 
                        HBO_modeled_mode_by_purpose_alt2,
                        HBW_modeled_mode_by_purpose_alt2,
                        NHB_modeled_mode_by_purpose_alt2) 
...
alternative_model <- model_compare_alt[c(9:11),]

overall_compare <- rbind(existing_model, alternative_model)

overall_compare

overall_compare_formatted <- overall_compare %>%
  rename(`Trip Type` = purpose) %>%
  mutate(`Percent Bike` = round(pct_bike * 100, 1),
         `Percent SOV` = round(pct_SOV * 100, 1),
         `Percent HOV` = round(pct_HOV * 100, 1),
         `Percent Transit` = round(pct_transit * 100, 1),
         `Percent Walk` = round(pct_walk * 100, 1)) %>% 
  .[,-c(2:6)] %>%
  tibble()

pdf("graphics/modesharetable.pdf", width = 8, height = 4)
grid.table(overall_compare_formatted) 
dev.off()


```

# Calculate changes from existing to alternative

```{r}
attach(overall_compare)

pct_change_bike <- rbind("pct_change_bike",
                         (pct_bike[4] - pct_bike[1]) / pct_bike[1],
                         (pct_bike[5] - pct_bike[2]) / pct_bike[2],
                         (pct_bike[6] - pct_bike[3]) / pct_bike[3])

pct_change_SOV <- rbind("pct_change_SOV",
                         (pct_SOV[4] - pct_SOV[1]) / pct_SOV[1],
                         (pct_SOV[5] - pct_SOV[2]) / pct_SOV[2],
                         (pct_SOV[6] - pct_SOV[3]) / pct_SOV[3])

pct_change_HOV <- rbind("pct_change_HOV",
                         (pct_HOV[4] - pct_HOV[1]) / pct_HOV[1],
                         (pct_HOV[5] - pct_HOV[2]) / pct_HOV[2],
                         (pct_HOV[6] - pct_HOV[3]) / pct_HOV[3])

pct_change_transit <- rbind("pct_change_transit",
                         (pct_transit[4] - pct_transit[1]) / pct_transit[1],
                         (pct_transit[5] - pct_transit[2]) / pct_transit[2],
                         (pct_transit[6] - pct_transit[3]) / pct_transit[3])


pct_change_walk <- rbind("pct_change_walk",
                         (pct_walk[4] - pct_walk[1]) / pct_walk[1],
                         (pct_walk[5] - pct_walk[2]) / pct_walk[2],
                         (pct_walk[6] - pct_walk[3]) / pct_walk[3]) 

detach(overall_compare)

pct_change_modes <- as.data.frame(
  cbind(rbind("TripType","HBO", "HBW", "NHB"),
        pct_change_bike,
        pct_change_SOV,
        pct_change_HOV,
        pct_change_transit,
...
colnames(pct_change_modes) <- pct_change_modes[1,] 
pct_change_modes <-  pct_change_modes[-1,] %>%
  mutate(pct_change_bike = round(as.numeric(pct_change_bike)*100, 1),
         pct_change_SOV = round(as.numeric(pct_change_SOV) * 100, 1),
         pct_change_HOV = round(as.numeric(pct_change_HOV) * 100, 1),
         pct_change_transit = round(as.numeric(pct_change_transit) * 100, 1),
         pct_change_walk = round(as.numeric(pct_change_walk) * 100, 1))


```

# Overall changes in mode share, regardless of purpose

```{r}
mode_share_overall <- data.frame(
  mode = rep(c('bike', 'SOV', 'HOV', 'transit', 'walk'), times = 2),
  condition = rep(c('existing', 'alternative'), each = 5),
  value = c(
    (sum(skims$n_bike_HBO) + sum(skims$n_bike_HBW) + sum(skims$n_bike_NHB)) / 
                 (sum(skims$HBO_flow) + sum(skims$HBW_flow) + sum(skims$NHB_flow)),
  (sum(skims$n_SOV_HBO) + sum(skims$n_SOV_HBW) + sum(skims$n_SOV_NHB)) / 
                 (sum(skims$HBO_flow) + sum(skims$HBW_flow) + sum(skims$NHB_flow)),
  (sum(skims$n_HOV_HBO) + sum(skims$n_HOV_HBW) + sum(skims$n_HOV_NHB)) / 
                 (sum(skims$HBO_flow) + sum(skims$HBW_flow) + sum(skims$NHB_flow)),
  (sum(skims$n_transit_HBO) + sum(skims$n_transit_HBW) + sum(skims$n_transit_NHB)) / 
                 (sum(skims$HBO_flow) + sum(skims$HBW_flow) + sum(skims$NHB_flow)),
  (sum(skims$n_walk_HBO) + sum(skims$n_walk_HBW) + sum(skims$n_walk_NHB)) / 
                 (sum(skims$HBO_flow) + sum(skims$HBW_flow) + sum(skims$NHB_flow)),
  (sum(skims_alt$n_bike_HBO) + sum(skims_alt$n_bike_HBW) + sum(skims_alt$n_bike_NHB)) / 
                 (sum(skims_alt$HBO_flow) + sum(skims_alt$HBW_flow) + sum(skims_alt$NHB_flow)),
  (sum(skims_alt$n_SOV_HBO) + sum(skims_alt$n_SOV_HBW) + sum(skims_alt$n_SOV_NHB)) / 
                 (sum(skims_alt$HBO_flow) + sum(skims_alt$HBW_flow) + sum(skims_alt$NHB_flow)),
  (sum(skims_alt$n_HOV_HBO) + sum(skims_alt$n_HOV_HBW) + sum(skims_alt$n_HOV_NHB)) / 
                 (sum(skims_alt$HBO_flow) + sum(skims_alt$HBW_flow) + sum(skims_alt$NHB_flow)),
  (sum(skims_alt$n_transit_HBO) + sum(skims_alt$n_transit_HBW) + sum(skims_alt$n_transit_NHB)) / 
                 (sum(skims_alt$HBO_flow) + sum(skims_alt$HBW_flow) + sum(skims_alt$NHB_flow)),
  (sum(skims_alt$n_walk_HBO) + sum(skims_alt$n_walk_HBW) + sum(skims_alt$n_walk_NHB)) / 
                 (sum(skims_alt$HBO_flow) + sum(skims_alt$HBW_flow) + sum(skims_alt$NHB_flow)))
  )

ggplot(mode_share_overall, aes(fill=mode, x=value, y = condition)) +
    geom_bar(position="stack", stat="identity") +
  theme_classic()

ggsave("graphics/mode_share_overall.png")
```
...
overall_compare_wide <- overall_compare %>%
  mutate(TripType = case_when(purpose %in% c("HBO_model 2", "HBO_model 2 alt") ~ "HBO",
                              purpose %in% c("HBW_model 2", "HBW_model 2 alt") ~ "HBW",
                              purpose %in% c("NHB_model 2", "NHB_model 2 alt") ~ "NHB"),
         condition = case_when(purpose %in% c("HBO_model 2 alt",
                                              "HBW_model 2 alt", 
                                              "NHB_model 2 alt") ~ "alternative",
                               purpose %in% c("HBO_model 2",
                                              "HBW_model 2", 
                                              "NHB_model 2") ~ "existing")) %>%
  t() %>%
  as.data.frame()

colnames(overall_compare_wide) <- paste0(overall_compare_wide[7,],"_",
                                         overall_compare_wide[8,])  

overall_compare_wide <- overall_compare_wide[-c(1,7:8),] %>%
    mutate_at(c(1:6), as.numeric) %>%
  mutate(mode = rownames(.)) %>%
  arrange(HBO_existing) %>%
  pivot_longer(cols = HBO_existing:NHB_alternative, names_to = "TripType")

ggplot(overall_compare_wide) +
  geom_bar(aes(x = TripType,
               y = value * 100,
               fill = mode), stat = 'identity') +
  scale_fill_brewer(name = "Mode",
                    labels = c("Bike","HOV","SOV","Transit","Walk")) +
  scale_x_discrete(labels = c("HBO - Alternative", "HBO - Existing",
                              "HBW - Alternative", "HBW - Existing",
                              "NHB - Alternative", "NHB - Existing")) +
  scale_y_continuous(name = "Percent Mode Share") +
  coord_flip() +
  theme(panel.background = element_blank(),
        text = element_text(size = 18))

ggsave("graphics/modeshare_bar.jpeg", height = 7, width = 14)

pct_change_modes_t <- pct_change_modes %>%
  t()

colnames(pct_change_modes_t) <- pct_change_modes_t[1,] 
pct_change_modes_t <- pct_change_modes_t[-1,] %>% 
  as.data.frame() %>%
...
HBW_color <- "#1f78b4"
NHB_color <- "#b2df8a"
legend_colors <- c("HBO" = HBO_color, "HBW" = HBW_color, "NHB" = NHB_color)

ggplot(pct_change_modes_t) +
  geom_segment(aes(y = 0, yend = 0,
                   x = .8, xend = 5.2), color = "grey69") +
  geom_segment(aes(x = pos+.2, xend = pos+.2,
                   y = 0, yend = HBO, color = "HBO"), 
               size = 2.5) +
  geom_point(aes(x = pos+.2, y = HBO, color = "HBO"), size = 8) +
  geom_segment(aes(x = pos, xend = pos,
                   y = 0, yend = HBW, color = "HBW"),
               size = 2.5) +
  geom_point(aes(x = pos, y = HBW, color = "HBW"), size = 8) +
  geom_segment(aes(x = pos-.2, xend = pos-.2,
                   y = 0, yend = NHB, color = "NHB"),
               size = 2.5) +
  geom_point(aes(x = pos-.2, y = NHB, color = "NHB"), size = 8) +
  scale_x_continuous(name = "Mode",
                     breaks = seq(1,5, by = 1),
                     labels = c("Bike","SOV","HOV","Transit","Walk")) +
  scale_y_continuous(name = "Percent Change in Modeshare") +
...
ggsave("graphics/modeshare_change.jpeg", height = 10, width = 14)

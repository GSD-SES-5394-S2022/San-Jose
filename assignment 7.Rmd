...
hh_pers_trips <- left_join(sanjose_hhs, trips_by_purpose) %>%
  ...
  mutate(ratio_wfh = WFH_exh / WRKCOUNT,
         wfh_int_ = case_when(ratio_wfh = 0 ~ 'no',
                             ratio_wfh != 0 & ratio_wfh <= 0.25 ~ 'low',
                             ratio_wfh <= 0.5 & ratio_wfh > 0.25 ~ 'mid',
                             TRUE ~ 'high'))
...
existing_zones <- here("existing",
  ...
  mutate(wfh_int = case_when(wfh_ratio = 0 ~ 'no',
                             wfh_ratio > 0 & wfh_ratio <= 0.25 ~ 'low',

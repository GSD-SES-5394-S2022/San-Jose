...
ex_hbo_attr <- dotdensity(existing_geo, "hbo_attr", 3000, 
                          "attr", "Existing HBO Attraction")
alt_hbo_attr <- dotdensity(alternative_geo, "hbo_attr", 3000, 
...
ex_hbw_attr <- dotdensity(existing_geo, "hbw_attr", 3000, 
                          "attr", "Existing HBW Attraction")
alt_hbw_attr <- dotdensity(alternative_geo, "hbw_attr", 3000, 
...
ex_nhb_attr <- dotdensity(existing_geo, "nhb_attr", 3000, 
                          "attr", "Existing NHB Attraction")
alt_nhb_attr <- dotdensity(alternative_geo, "nhb_attr", 3000, 
...
ggplot(existing_geo) +
  geom_sf(aes(fill = wfh_level), color = NA) +
  scale_fill_manual(name = "WFH Ratio",
                    values = map_color, 
...
ggplot(existing_geo) +
  geom_sf(color = "white", fill = "black") +
  geom_sf(data = hbo_prod_dot, color = "purple", size = .7, alpha = .3) +
  geom_sf(data = hbo_attr_dot, color = "green", size = .6, alpha = .1) +
  theme_void()


# chloropleths for existing/alt differences
sj_trips <- existing_geo[, c(1:2, 55:66)] %>%
  st_join(alternative_geo[, c(1:2, 61:73)]) %>%
  mutate(hbo_prod = round((hbo_prod.y - hbo_prod.x) / hbo_prod.x * 100),
         hbo_attr = round((hbo_attr.y - hbo_attr.x) / hbo_attr.x * 100),
         hbw_prod = round((hbw_prod.y - hbw_prod.x) / hbw_prod.x * 100),
         hbw_attr = round((hbw_attr.y - hbw_attr.x) / hbw_attr.x * 100),
         nhb_prod = round((nhb_prod.y - nhb_prod.x) / nhb_prod.x * 100),
         nhb_attr = round((nhb_attr.y - nhb_attr.x) / nhb_attr.x * 100)) %>%
...
chlormap <- function(df, variable, pal = map_color2, title = NULL){
  ggplot(df) +
    geom_sf(aes(fill = df[,variable][[1]]), alpha = .7, color = NA) +
